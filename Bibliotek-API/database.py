...
with open('../bøker.csv', "r") as file:
  ...
  for bok in bøker:
    d_bok = bok.split(",")
    bokliste.append({"tittel": d_bok[0], "forfatter": d_bok[1], "ISBN": d_bok[2], "nummer": d_bok[3]})
cur.executemany("UPDATE bøker SET tittel = ?, forfatter = ?, ISBN = ? WHERE nummer = ?", [(bok["tittel"], bok["forfatter"], bok["ISBN"], bok["nummer"]) for bok in bokliste])
con.commit()
