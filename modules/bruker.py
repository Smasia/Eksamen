...
cur.execute("""CREATE TABLE brukere(
...
def registrer():
  ...
  if request.method == "POST":
    ...
    requests.post("http://192.168.10.27/registrer", json={"navn": navn, "passord": passord})
    return redirect("/logg_inn")
...
def logg_inn():
  ...
  if request.method == "POST":
    navn = request.form.get("navn")
    passord = request.form.get("passord")
    response = requests.post("http://192.168.10.27/logg_inn", json={"navn": navn, "passord": passord})
